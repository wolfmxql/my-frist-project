...
LastEditTime: 2025-07-18 15:02:35
...
def DataInterpFill(lst):
    """
    @description  : 对lst中的缺失值(np.nan)进行线性填充
    ---------
    @param  :
    -------
    @Returns  :
    ...
    if len(non_nan_indices) != 0:
        ...
        # 对所有索引进行插值
        arr_interpolated = np.interp(np.arange(len(arr)), non_nan_indices, non_nan_values)
        # 输出结果
        ##20250418 print(arr_interpolated)
    else:
        arr_interpolated = np.nan_to_num(arr, nan=0)
    # 返回填充后的数据
    return arr_interpolated


lst = [1, np.nan, np.nan, np.nan, np.nan, np.nan]
arr_interpolated = DataInterpFill(lst)
print(arr_interpolated)


newbjnr = '仪表故障&&输差较大，疑似设备故障或始站站内产生泄漏'

print('泄漏' in newbjnr)
print('站内产生' not in newbjnr)
print('泄漏' in newbjnr and '站内产生' not in newbjnr)
        ...
        with cx_Oracle.connect(user=username, password=password, dsn=dsn) as connection:
            with connection.cursor() as cursor:
                ...
                if result[0] > 0:
                    ...
                    # 创建表的SQL语句
                    create_table_sql = f"create table {table_name} (id number primary key, pipename varchar2(50), runtime varchar2(100), runtimelength number, szflow_mean number, mzflow_mean number, flowloss_mean number, weekflowloss number, cumuszflow number, cumumzflow number, cumuflowloss number, faultstate varchar2(255), faulttimelength varchar2(255))"
...
def DataStateAnalysis(Rundata_df, AnalysisData_Path, WeekFlowloss_mean):
    """
    ...
    try:
        ...
        # 开始于结束时间
        StartTime_str = timestamp_to_hour_minute_simple(int(Rundata_df.iloc[0]['TIME']))
        EndTime_str = timestamp_to_hour_minute_simple(int(Rundata_df.iloc[-1]['TIME']))
        RunTime = f"{StartTime_str}-{EndTime_str}"
        # 管道名称
        PipeName = Rundata_df.iloc[-1]['PIPENAME']
        # 故障文件路径
        FaultState_Path = 'Data/RunInfo/FaultState.csv'
        # 根据始站排量的数据计算运行时长
...
def query_oracle(query_str, oracleinfo):
    """
    @description  : 从oracle中查询结果
    ---------
    @param  :   query_str:  Oracle查询语句
                oracleinfo: Oracle连接信息
    -------
    @Returns  :
    -------
    """
...
def WeekFlowloss(WeekAnalysisData_df):
    ...
    EndTime = int(WeekAnalysisData_df.iloc[-1]['RCORDTIME'])
...
def InfluxbDB_DataAnalysis(pipeconfig, oracleinfo):
    ...
    if len(Pipe_Table_dict) > 0:
        Pipe_Table = Pipe_Table_dict[-1]['GROUPREG']
        strSql = f'SELECT * FROM "{Pipe_Table}" WHERE time > now() - 8d order by time asc tz("Asia/Shanghai")'
        # 提出influxdb数据
        InfluxDbData = client.query(strSql, database=database1)
        InfluxListData = list(InfluxDbData.get_points())
...
def CumulataWeekData(AnalysisData_Path, pipeconfig, oracleinfo):
    ...
    AnalysisData_df = pd.read_csv(AnalysisData_Path, encoding='utf-8-sig')
    # 提取周数据信息
    Recordtime_lst = AnalysisData_df['RECORDTIME'].values()
    # 检查数据记录是否超过一周
...
def ExtractRundata(oracleinfo, pipeconfig):
    ...
    if len(Pipe_Table_dict) > 0:
        Pipe_Table = Pipe_Table_dict[-1]['GROUPREG']
        strSql = f"SELECT * FROM '{Pipe_Table}' WHERE time > now() -8h order by time asc tz('Asia/Shanghai')"
        # 提出influxdb数据
        InfluxDbData = client.query(strSql, database=database1)
        InfluxListData = list(InfluxDbData.get_points())
        InfluxDB_Rundata_df = pd.DataFrame(InfluxListData)
    else:
        InfluxDB_Rundata_df = f"未找到{pipeconfig['PIPENAME']}管道的数据表"
...
def CheckRunState():
    ...
    try:
        ...
        if OracleINFO_df.shape[0] > 0:
            ...
            if Pipeconfig_df != None and Pipeconfig_df.shape[0] > 0:
                ...
                for Pipeconfig in Pipeconfig_dict:
                    ...
                    if type(Rundata_df) != type('string'):
                        # 写入csv文件
                        Rundata_df.to_csv(NowDataPath, encoding='utf-8-sig')
                        if Rundata_df.shape[0] > 600:
                            ...
                            if type(WeekFlowloss_mean) == type('string'):
                                WeekFlowloss_mean = '/'
                            # 计算班次运行状态
                            DataStateAnalysis(Rundata_df, AnalysisData_Path, WeekFlowloss_mean)
                        else:
                            print(f'本班次管道{PipeName}数据异常')
                    else:
                        print(f'本班次管道{PipeName}数据异常')
            else:
...
class MonitorApp:
    def __init__(self, root):
        ...
        # 启动按钮
        self.start_button = tk.Button(root, text="启动监控", command=self.start_monitor)
        self.start_button.pack(pady=10)

        # 停止按钮
        self.stop_button = tk.Button(root, text="停止监控", command=self.stop_monitor, state=tk.DISABLED)
        self.stop_button.pack(pady=5)

        # 清空按钮
        self.clear_button = tk.Button(root, text="清空输出", command=self.clear_output)
        ...
        # 添加自动启动
        self.root.after(100, self.start_monitor)
    ...
    def RunMonitor(self):
        ...
        if self.last_start_time is not None:
            ...
            print(f"距离上次执行：{elapsed_time:.2f}秒")
        ...
        print(f"开启监控：{time.strftime('%H:%M:%S')}")

        CheckRunState()

        print(f"本次监控完成时间：{time.strftime('%H:%M:%S')}")

        if self.monitoring:
            next_wait_time = self.interval - (time.time() - self.last_start_time)

            # 确保等待时间为正
            next_wait_time = max(0, next_wait_time)

            print(f"下次监控将在 {next_wait_time:.2f} 秒后执行")

            # 设置下一次执行
            self.root.after(int(next_wait_time * 1000), self.RunMonitor)
